// sandbox ourselves to guarantee we don't interfere with OnDemand platform --Abhishek122344
// JS internals
jQuery(function($) {
...
var augmentCallDetailsEntry = function() {
	...
	row += "<td>Product*:<form name=CallProdDetailNew action="" id=CallProdDetailNew method=post><input name=CallProdDetailNew.Name size="" tabindex="3" type=text value="" class=inputControl id=CallProdDetailNew.Name /><input type=hidden id=CallProdDetailNew.Product Id name=CallProdDetailNew.Product Id tabindex="-1" value=""><input type=hidden id=CallProdDetailNew.Name.hidden name=CallProdDetailNew.Name.hidden tabindex="-1" value=""><script type=text/javascript>function newPopup(url){popupWindow = window.open(url,'popUpWindow','height=700,width=800,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no,status=yes')}</script>
                <a href=JavaScript:newPopup('https://secure-ausomxapa.crmondemand.com/OnDemand/user/AssocProductPopup?mapBC=Pharma+Call+Products+Detailed&OACTRL=Name&ophi=CallProdDetailNew.Product+Id&pfid=CallProdDetailNew&OMTHD=AssocPopup&OMTGT=PopupSearchList&assocInit=Y&opht=4&OAOBJ=Call+ProdDetail&mapField=Name&ophd=CallProdDetailNew.Name&ophpd=1&disableclear=Y&ophrssocProductPopup&assocval=&ParentType=Edit');>Test</a></input></td>";
